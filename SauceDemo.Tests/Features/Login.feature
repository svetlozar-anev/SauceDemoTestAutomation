Feature: Login Functionality
  ...
  Scenario: UC-001: Login fails with empty credentials
  ...
  Scenario: UC-002: Login fails with missing password
    Given I am on the login page
  ...
  Scenario: UC-004: Login fails with locked out user
    Given I am on the login page
  ...
  Scenario: UC-005: Login fails with wrong password
    Given I am on the login page
    And I enter "standard_user" in the username field
  ...
  Scenario: UC-006: Login fails with missing username
    Given I am on the login page
    And I enter "secret_sauce" in the password field
    When I click the login button
    Then I should see the error message "Epic sadface: Username is required"

  Scenario: UC-007: Login fails with special characters in username and password
    Given I am on the login page
  ...
  Scenario: UC-008: Login fails with whitespace-only username and password
    Given I am on the login page
    And I enter "    " in the username field
    And I enter "    " in the password field
    When I click the login button
